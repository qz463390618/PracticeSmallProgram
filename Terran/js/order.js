    ...
    */
            ...
            if(len>0) {
                for (var i = 0; i < len; i++) {
                    ...
                    str += '<tr>' +
                        ...
                        '<td data-id="' + item.id + '"><a class="order-btn detail">查看订单详情</a>' + getBtns(item.status) + '</td>' +
                        '</tr>';
                }
            }
            else{
    ...
    $(document).on('click','.order-btn.done',function(){
        var $this=$(this),
            ...
            $p=$tips.find('p');
        var params={
            url:'order/delivery?XDEBUG_SESSION_START=19961',
            type:'post',
            data:{id:id},
            tokenFlag:true,
            sCallback:function(res) {
    ...
    /*查看订单详情*/
    $(document).on('click','.order-btn.detail',function(){
